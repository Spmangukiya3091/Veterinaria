...
const userRegister = async (req, res) => {
  ...
  if (user) {
    ...
    const userData = {
      ...
      role: req.body.role,
      profile: req.protocol + "://" + req.get("host") + `/profile/profile/${req.file.filename}`,
      isverified: false,
...
const updateUserProfile = async (req, res) => {
  ...
  if (user?.id === id && password === confirmPassword) {
    await Auth.update({ name, email, password, role, profile, phone, identification }, { where: { id: id } });
...
const changeUserPassword = async (req, res) => {
  ...
  const user = await Auth.findOne({ where: { id: id } });
  if (user?.id === id && user?.password === currentPassword && confirmPassword === password) {
  ...
  <a href="http://localhost:3000/updatepassword?token=${user.emailToken}" style="display:block; padding:15px 25px; background-color:#448EF6; color:#ffffff; border-radius:7px; text-decoration:none;">Verify Email</a>
    ...
    transporter.sendMail(mailoption, function (error, info) {
      if (error) {
        ...
        Auth.update({ emailToken: token }, { where: { email: user.email } });
  ...
  <a href="href="http://localhost:3000/updatepassword?token=${veterinarian.emailToken}" style="display:block; padding:15px 25px; background-color:#448EF6; color:#ffffff; border-radius:7px; text-decoration:none;">Verify Email</a>
    ...
    transporter.sendMail(mailoptions, function (error, info) {
      if (error) {
        ...
        Veterinarian.update({ emailToken: token }, { where: { email: veterinarian.email } });
        console.log("verification email sent successfully ", info.response);
        res.status(200).send({
          message: "verification link sent successfully",
        });
...
const updatePassword = async (req, res, next) => {
  try {
    ...
    const veterinarian = await Veterinarian.findOne({ emailToken: token });
    ...
    } else if (veterinarian.emailToken !== null) {
      if (password === confirmPassword) {
