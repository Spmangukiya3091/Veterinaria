...
const createPet = async (req, res) => {
  try {
    const pet = {
      ...
      Species: req.body.Species === "" || !req.body.Species ? null : req.body.Species,
...
//       attributes: ["id", "name", "owner", "ownerId", "age", "dob", "sex", "age", "Species", "breed", "hair", "color", "exploration", "status"],
//       include: [
//         {
//           model: Appointment,
//           as: "petAppointmentData",
...
const getAllPets = async (req, res) => {
  try {
    const petsList = await Pet.findAll({
      attributes: ["id", "name", "ownerId", "age", "dob", "sex", "age", "Species", "breed", "hair", "color", "exploration", "status"],
      include: [
        {
          model: Owner,
          as: "ownerByPet", // Use the correct alias defined in the association
          attributes: ["id", "name", "surname"], // Fetch owner id, name, and surname
        },
        {
          model: Appointment,
          as: "petAppointmentData",
    ...
    const petsListWithOwners = petsList.map((pet) => {
      const ownerDetails = pet.ownerByPet || {}; // Use an empty object if ownerByPet is null

      return {
        id: pet.id,
        name: pet.name,
        ownerId: pet.ownerId,
        owner: `${ownerDetails.name || ""} ${ownerDetails.surname || ""}`,
        sex: pet.sex,
        dob: pet.dob,
        age: pet.age,
        species: pet.Species,
        ...
        status: pet.status,
        exploration: pet.exploration,
        petAppointmentData: pet.petAppointmentData,
      };
    });

    res.status(200).send({
      message: "petsList",
      petsList: petsListWithOwners,
    });
  } catch (error) {
    console.error("Error fetching pets:", error);
    res.status(500).json({ message: "Internal server error" });
...
//     const completeAppointments = pets.petAppointmentData.filter((appointment) => appointment.status === "complete").length;
...
const getSinglePet = async (req, res) => {
  try {
    ...
    // Calculate total appointment count
    const totalAppointments = pet.petAppointmentData.length;

    // Calculate complete appointment count
    const completeAppointments = pet.petAppointmentData.filter((appointment) => appointment.status === "complete").length;

    // Find the last appointment date and time
    let lastAppointment = null;
    if (pet.petAppointmentData.length > 0) {
      pet.petAppointmentData.sort((a, b) => new Date(b.date) - new Date(a.date));
      lastAppointment = {
        date: pet.petAppointmentData[0].date,
      };
    }
    ...
    const petResponse = {
      ...
      rating: pet?.petAppointmentData[0]?.rating,
    };
