...
const createVeterinarian = async (req, res) => {
  try {
    const veterinarian = {
      avatar: req.file ? req.protocol + "://" + req.get("host") + `/profile/veterinarian/${req.file.filename}` : null,
      name: req.body.name,
      surname: req.body.surname,
      speciality: req.body.speciality === "" || !req.body.speciality ? null : req.body.speciality,
      ...
      workingDays: req.body.workingDays === undefined ? [] : req.body.workingDays,
    ...
    } else if (veterinarian.confirmPassword === "" || veterinarian.confirmPassword === null) {
      res.status(201).send({
    ...
    } else if (veterinarian.department === "" || veterinarian.department === null) {
      res.status(201).send({
        message: "department is required",
      });
    } else if (veterinarian.workingDays === "" || veterinarian.workingDays === null) {
      res.status(201).send({
        message: "workingDays is required",
      });
    } else if (veterinarian.start_time === "" || veterinarian.start_time === null) {
      res.status(201).send({
...
const updateVeterinarian = async (req, res) => {
  try {
    ...
    const veterinarian = {
      avatar: req.file ? req.protocol + "://" + req.get("host") + `/profile/veterinarian/${req.file.filename}` : req.body.avatar,
      name: req.body.name,
      surname: req.body.surname,
...
const veterinarianFilter = async (req, res) => {
  try {
    ...
    } else if (req.query.startDate && req.query.endDate && !req.query.speciality) {
      const filteredRecords = await Veterinarian.findAll({
        where: {
          createdAt: {
            [Op.between]: [startDate, endDate],
    ...
    } else if (req.query.speciality && !req.query.startDate && !req.query.endDate) {
      const filteredRecords = await Veterinarian.findAll({
        where: {
          speciality,
        },
...
const veterinarianExcelFile = async (req, res) => {
  try {
    const data = await Veterinarian.findAll();
    ...
    workbook.xlsx.write(res, { response: res, fileName: "payment.xlsx" });
  } catch (error) {
  ...
  try {
    ...
    veterinarians.forEach((vet) => {
      vet.vetsAppointment.forEach((appointment) => {
        data.push({
          name: vet.name,
          start: new Date(`${appointment.date.toISOString().split("T")[0]} ${appointment.scheduleStart}`),
          end: new Date(`${appointment.date.toISOString().split("T")[0]} ${appointment.scheduleEnd}`),
        });
      });
    });
...
const getSingleVeterinarians = async (req, res) => {
  try {
    ...
    const pendingAppointments = veterinarian.vetsAppointment.filter((appointment) => appointment.status === "pending").length;
  ...
  try {
    ...
    if (req.query.startDate && req.query.endDate) {
      const matchedOwners = owners
        .filter((owner) => {
          return (
            veterinarians.some((vet) => vet.ownerId === owner.id) && new Date(owner.createdAt) >= startDate && new Date(owner.createdAt) <= endDate
          );
        })
        .map(({ id, name, dob, surname, phone_1, phone_2, doc_identity, email, address, department, district }) => ({
          id,
          name,
          dob,
          surname,
    ...
    if (req.query.startDate && req.query.endDate) {
      const matchedPets = pets
        .filter((pet) => {
          ...
          return veterinarians.some((vet) => vet.petId === pet.id) && new Date(pet.createdAt) >= startDate && new Date(pet.createdAt) <= endDate;
        })
        .map(({ id, name, owner, sex, age, Species, breed, hair, color, status, exploration, dob }) => ({
          id,
          name,
          owner,
          sex,
