...
const Historical = ({ id }) => {
  ...
  const appointmentHistory = useGetPetAppoinmentQuery(searchQuery, { refetchOnMountOrArgChange: true });

  useEffect(() => {
    if (!appointmentHistory.isLoading && id !== undefined) {
      setLoading(false);
      setData(appointmentHistory?.data?.appointments);
    } else if (appointmentHistory.isError) {
      setError(false);
      setLoading(false);
    }
                      ...
                      currentPosts.map(({ id, pet, scheduleStart, scheduleEnd, date, status, ACCIONES }, i) => (
                        <tr key={i}>
                          ...
                          <td className="text-end">
                            <a
                              href={`/customerservice/citas-view/${id}`}
