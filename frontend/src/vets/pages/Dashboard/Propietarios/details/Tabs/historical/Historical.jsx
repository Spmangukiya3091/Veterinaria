...
const Historical = ({ id }) => {
  ...
  const petAppointmentsOfOwner = useGetPetAppoinmentOfOwnerQuery(searchQuery, { refetchOnMountOrArgChange: true });

  useEffect(() => {
    if (!petAppointmentsOfOwner.isLoading && id !== undefined) {
      setLoading(false);
      setData(petAppointmentsOfOwner?.data?.appointments);
    } else if (petAppointmentsOfOwner.isError) {
      setError(false);
      setLoading(false);
    }
  }, [petAppointmentsOfOwner, id]);
          ...
          <div className="historical-table-container">
            <div className="card card-flush">
              <div className="card-header align-items-center py-5 gap-2 gap-md-5 gap-2 gap-md-5">
                ...
                <p className="text-gray-400 fw-bolder fs-5">{data?.appointments?.length} Pacientes</p>
              </div>
